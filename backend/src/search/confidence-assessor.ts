...
export function assessConfidence(results: RerankedResult[]): ConfidenceAssessment {
  ...
  const strongMatches = results.filter(r => r.finalScore > 0.4).length;

  let level: ConfidenceLevel;
  let score: number;

  if (topScore > 0.5 && strongMatches >= 2) {
    level = 'high';
    score = Math.min(Math.round(topScore * 100 + 10), 95);
  } else if (topScore > 0.35 || strongMatches >= 1) {
    level = 'medium';
    score = Math.min(Math.round(avgScore * 100 + 20), 75);
  } else {
    level = 'low';
